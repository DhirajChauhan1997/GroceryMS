...
class SubCategoryController extends CI_Controller
{
	...
	public function index()
	{
		if ($this->session->userdata('level') === 'admin') {
			...
			$rowsCount = $this->SubCategoryModel->getRows($conditions);

			// Pagination config
			$config['base_url']    = base_url() . 'index.php/SubCategoryController/index/';
			...
			$data['datalst'] = $this->SubCategoryModel->getRows($conditions);
			$data['title'] = 'Members List';

			$this->load->view('layouts/header.php');
			$this->load->view('admin/SubCategory/index.php', $data);
		...
		if ($this->session->userdata('level') === 'admin') {
			...
			if ($this->input->post('formSubmit')) {
				...
				if (!empty($_FILES['cat_logo']['name'])) {
					...
					$file_name = $this->SubCategoryModel->generateRandomString() . $_FILES['cat_logo']['name'];
					...
					// $randomName = $this->SubCategoryModel->generateRandomString();
					...
					$this->form_validation->set_rules('sub_cat_name', 'Role Name', 'trim|required');
					$this->form_validation->set_rules('cat_id', 'Role Name', 'trim|required');
					...
					if ($this->form_validation->run() == true) {
						...
						// Insert member data
						$insert = $this->SubCategoryModel->insert($memData);
				...
				$this->form_validation->set_rules('sub_cat_name', 'Role Name', 'trim|required');
				//$this->form_validation->set_rules('cat_id', 'Role Name', 'trim|required');
				//$this->form_validation->set_rules('cat_logo', 'Role Name', 'trim|required');
			...
			$this->load->view('layouts/header.php');
			$this->load->view('admin/SubCategory/createNewSubCategory.php', $data);
			$this->load->view('layouts/footer.php');
		} else {
			redirect('authentication');
			echo "Access Denied";
	...
	public function edit($id)
	{
		...
		// Get member data
		$memData = $this->SubCategoryModel->getRows(array('id' => $id));
		$categore = $this->SubCategoryModel->getCategoryById($id);
		$data['category'] = $this->SubCategoryModel->getAllCategory();
		...
		if ($this->input->post('formSubmit')) {
			...
			//$this->session->set_userdata('success_msg', 'Inside MemSubmit.');
			if (!empty($_FILES['cat_logo']['name'])) {
				...
				if ($this->form_validation->run() == true) {
					// Update member data
					$update = $this->SubCategoryModel->update($memData, $id);
			...
			if ($this->form_validation->run() == true) {
				...
				if ($update) {
					$this->session->set_userdata('success_msg', 'Member has been updated successfully.');
					redirect('/SubCategoryController/index');
				} else {
					$data['error_msg'] = 'Some problems occured, please try again.';
				}
			}
	...
	public function view($id)
	{
		...
		if (!empty($id)) {
			$data['member'] = $this->SubCategoryModel->getRows(array('sub_cat_id' => $id));;
			...
			redirect('index.php/SubCategoryController/index');
		}
	}
}
