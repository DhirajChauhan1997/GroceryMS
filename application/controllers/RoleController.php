...
class RoleController extends CI_Controller
{
	...
	public function index()
	{
		if ($this->session->userdata('level') === 'admin') {
			...
			$rowsCount = $this->RoleModel->getRows($conditions);

			// Pagination config
			$config['base_url']    = base_url() . 'index.php/RoleController/index/';
			...
			$data['datalst'] = $this->RoleModel->getRows($conditions);
			$data['title'] = 'Members List';

			$this->load->view('layouts/header.php');
			$this->load->view('admin/Role/index.php', $data);
	...
	public function createNewUserRole()
	{
		if ($this->session->userdata('level') === 'admin') {
			...
			if ($this->input->post('formSubmit')) {
				...
				if ($this->form_validation->run() == true) {
					...
					// Insert member data
					$insert = $this->RoleModel->insert($memData);
			...
			$this->load->view('layouts/header.php');
			$this->load->view('admin/Role/createNewRole.php');
			$this->load->view('layouts/footer.php');
		} else {
			redirect('authentication');
			echo "Access Denied";
	...
	public function edit($id)
	{
		...
		// Get member data
		$memData = $this->RoleModel->getRows(array('id' => $id));
		...
		if ($this->input->post('memSubmit')) {
			...
			if ($this->form_validation->run() == true) {
				// Update member data
				$update = $this->RoleModel->update($memData, $id);
		...
		$this->load->view('admin/Role/createNewRole.php', $data);
	...
	public function view($id)
	{
		...
		// Check whether member id is not empty
		if (!empty($id)) {
			$data['member'] = $this->RoleModel->getRows(array('id' => $id));;
			...
			redirect('index.php/RoleController/index');
		}
	}
}
