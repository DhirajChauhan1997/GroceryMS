...
class CityController extends CI_Controller
{
	...
	public function index()
	{
		if ($this->session->userdata('level') === 'admin') {
			...
			$rowsCount = $this->CityModel->getRows($conditions);

			// Pagination config
			$config['base_url']    = base_url() . 'index.php/CityController/index/';
			...
			$data['datalst'] = $this->CityModel->getRows($conditions);
			$data['title'] = 'Members List';

			$this->load->view('layouts/header.php');
			$this->load->view('admin/City/index.php', $data);
	...
	{
		if ($this->session->userdata('level') === 'admin') {
			$data['states'] = $this->CityModel->getAllStates();
			...
			if ($this->input->post('formSubmit')) {
				...
				$this->form_validation->set_rules('state_id', 'City Name', 'trim|required');
				...
				if ($this->form_validation->run() == true) {
					...
					// Insert member data
					$insert = $this->CityModel->insert($memData);
			...
			$this->load->view('layouts/header.php');
			$this->load->view('admin/City/createNewCity.php', $data);
			$this->load->view('layouts/footer.php');
		} else {
			redirect('authentication');
			echo "Access Denied";
	...
	public function edit($id)
	{
		...
		// Get member data
		$memData = $this->CityModel->getRows(array('id' => $id));
	...
	public function view($id)
	{
		...
		// Check whether member id is not empty
		if (!empty($id)) {
			$data['member'] = $this->CityModel->getRows(array('id' => $id));;
			...
			redirect('index.php/CityController/index');
		}
	}
}
