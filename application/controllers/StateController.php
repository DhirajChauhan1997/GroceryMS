...
class StateController extends CI_Controller
{
	...
	public function index()
	{
		if ($this->session->userdata('level') === 'admin') {
			...
			$rowsCount = $this->StateModel->getRows($conditions);

			// Pagination config
			$config['base_url']    = base_url() . 'index.php/StateController/index/';
			...
			$data['datalst'] = $this->StateModel->getRows($conditions);
			$data['title'] = 'Members List';

			$this->load->view('layouts/header.php');
			$this->load->view('admin/State/index.php', $data);
		...
		if ($this->session->userdata('level') === 'admin') {
			...
			if ($this->input->post('formSubmit')) {
				...
				if ($this->form_validation->run() == true) {
					...
					// Insert member data
					$insert = $this->StateModel->insert($memData);
			...
			$this->load->view('layouts/header.php');
			$this->load->view('admin/State/createNewState.php');
			$this->load->view('layouts/footer.php');
		} else {
			redirect('authentication');
			echo "Access Denied";
	...
	public function edit($id)
	{
		...
		// Get member data
		$memData = $this->StateModel->getRows(array('id' => $id));
		...
		if ($this->input->post('formSubmit')) {
			...
			if ($this->form_validation->run() == true) {
				// Update member data
				$update = $this->StateModel->update($memData, $id);
		...
		$this->load->view('admin/State/createNewState.php', $data);
	...
	public function delete($id)
	{
		...
		if ($id) {
			// Delete member
			$delete = $this->StateModel->delete($id);
	...
	public function view($id)
	{
		...
		// Check whether member id is not empty
		if (!empty($id)) {
			$data['member'] = $this->StateModel->getRows(array('id' => $id));;
			...
			redirect('index.php/StateController/index');
		}
	}
}
